...
public class MultipleMartingale implements IStrategy {
    ...
    public final static String STRATEGY_TAG = "MultipleMartingale";
...
//        rsiParams.put(Instrument.EURUSD, EURUSDRSIParams);
        ...
        mMartingaleParams.put(Instrument.GBPAUD, GBPAUDMartingaleParams);
        RSIParams GBPAUDRSIParams = new RSIParams();
        GBPAUDRSIParams.setrSIPeriod(Period.FIVE_MINS);
        rsiParams.put(Instrument.GBPAUD, GBPAUDRSIParams);
...
//        puts("totalPips: " + totalPips);
    ...
    private void tryAddPosition(Instrument instrument, ITick tick) throws JFException {
        ...
        int orderCount = mEngine.getOrders(instrument).size();
        long timeInterval = tick.getTime() - lastOrder.getCreationTime();
        MartingaleParams martingaleParams = mMartingaleParams.get(instrument);
        if (orderCount < martingaleParams.getAmountArray().length &&
                checkTimeInterval(timeInterval, orderCount) &&
                    checkCloseHigher(instrument, orderCommand, orderCount)) {
            double addAmount = (martingaleParams.getAmountArray())[orderCount];
            double addPrice;
            if (orderCommand.equals(IEngine.OrderCommand.BUY)) {
                addPrice = lastOrder.getOpenPrice() - martingaleParams.getGridPips(orderCount) * instrument.getPipValue();
...
//                puts("tryAddposition, sell, addprice: " + addPrice + ", addAmount: " + addAmount + ", ask: " + tick.getAsk());
    ...
    private boolean isHavePosition(Instrument instrument) throws JFException {
        return mEngine.getOrders(instrument).size() > 0;
    }
    ...
    private void handleOrderFilled(Instrument instrument) throws JFException {
        ...
        if (orderCount >= maxOrderCount) {
            sendEmail("WARNING!!! MAX ORDER COUNT",
                    instrument + " order count: " + orderCount);
        }
...
//        mConsole.getInfo().println(str);
    ...
    public class MartingaleParams {
        ...
        private double[] mAmountArray = {0.003, 0.005, 0.008, 0.008, 0.008, 0.008, 0.008, 0.008};
        ...
        public int getGridPips(int orderCount) {
            return mGridPipsArray[orderCount];
        }
